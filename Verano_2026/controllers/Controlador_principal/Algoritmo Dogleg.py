...
ruta_log = os.path.join(ruta_base, "dogleg_log.txt")
...
def fun_costo(p):
    ...
    for w in wp3d:
        x = w[0]
        y = w[1]
        z = 2 #Dejar fijo si se mueven demasiadas iteraciones
        dx = 1.0 - x
        dy = 1.0 - y
        yaw = mt.atan2(dy, dx)
        wp.append([x, y, z, yaw])
...
if __name__ == "__main__":
    ...
    escribir_log(f"Norma gradiente  : {np.linalg.norm(g):.6f}")
    escribir_log(f"Norma Hessiana   : {np.linalg.norm(H):.6f}")
    escribir_log(f"Radio final      : {Delta:.6f}")
    ...
    plt.plot(range(1, len(fitness_historial)+1),
            fitness_historial,
            marker='o',
            linewidth=2)

    plt.xlabel("Iteración Dogleg")
    plt.ylabel("Fitness (Costo)")
    plt.title("Convergencia del algoritmo Dogleg")
    plt.grid(True)

    plt.savefig(os.path.join(ruta_base,"fitness_por_iteracion.png"))
